...
function search(pageNum) {
	...
	target : $("#data_list"),
	template : $("#data_temp"),
	url : BASE.basePath + "/outBank/doList",
...
function searchBroken(pageNum) {
	...
	$("#ajaxPager").zPager({
		target : $("#data_list"),
		template : $("#data_temp"),
		url : BASE.basePath + "/outBank/doBrokenMedicineList",
		basePath : BASE.basePath,
		method : "POST",
...
function editor(id) {
	layer.open({
	    ...
	    zIndex:999,
	    moveType: 1,
	    content: BASE.basePath + '/outBank/toEditor' + (zUtils.isUndefined(id) ? "" : "?id=" + id)
	});
}
...
function selectMedicine(id) {
	...
	layer.open({
	    ...
	    zIndex:999,
	    moveType: 1,
	    content: BASE.basePath + '/outBank/toBrokenMedicineSelectList' + (zUtils.isUndefined(id) ? "" : "?id=" + id),
...
function del(brokenId, msg) {
	...
	layer.confirm("确定要删除[" + msg + "]吗?", {
		...
		$.ajax({
			type : "POST",
			url : BASE.basePath + '/outBank/doDel',
			data : {
				'id' : brokenId
			},
			success : function(rm) {
...
function initEditor() {
 ...
 $('#saveForm').zAction({
	url : BASE.basePath + "/outBank/doEditor",
	method : 'POST',
	before : function() {
		layer.load(2);
	},
...
function enable(obj) {
	...
	$.ajax({
		type : "POST",
		url : BASE.basePath + '/outBank/enable',
		data : {
			'id' : $(obj).val(),
			'value' : value
		},
