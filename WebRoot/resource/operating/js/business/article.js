...
function search(pageNum) {
	...
	target : $("#data_list"),
	template : $("#data_temp"),
	url : BASE.basePath + "/articleBack/doList",
...
function editor(id) {
	...
	var url = BASE.basePath + '/articleBack/toEdit' + (zUtils.isUndefined(id) ? "" : "?articleid=" + id);
...
function del(articleid, msg) {
	...
	layer.confirm("确定要删除[" + msg + "]吗?", {
		...
		$.ajax({
			type : "POST",
			url : BASE.basePath + '/articleBack/doDelete',
			data : {
				'articleid' : articleid
			},
			success : function(rm) {
...
function delall(fieldName) {
	...
	layer.confirm("确定要删除[" + "选中的文章" + "]吗?", {
		...
		$.ajax({
			...
			url : BASE.basePath + '/articleBack/doDelete',
			data : {
				'articleid' : checkedList.toString()
			},
			success : function(rm) {
				if (rm.result) {
					layer.msg(rm.message, {icon: 1});
					...
					strParameters.each(function() {
						...
						$('#article_' + articleId).fadeOut(200, function() {
...
function initEditor() {
    ...
    $('#saveForm').zAction({
	url : BASE.basePath + "/articleBack/save",
	method : 'POST',
	before : function() {
		layer.load(2);
	},
