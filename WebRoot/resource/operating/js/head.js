...
//siMenu('${son.columnId}','column_${sc.columnId}','${son.columnName}','<@b.basePath />${son.columnUrl}')
/**
 * id：菜单id
 * fid: 父Id
 * MENU_NAME:菜单名称
...
function editUserH(){
	 ...
	 diag.Drag=true;
	 diag.Title ="个人资料";
	 diag.URL = BASE.basePath + "/operating/toEditUser";
	 diag.Width = 380;
	 diag.Height = 450;
...
function initEditor() {
	...
	$('#saveForm').zAction({
		url : BASE.basePath + "/operating/saveEditUser",
		method : 'POST',
		before : function() {
			$('#saveForm').hide();
			$('#save_loading').show();
