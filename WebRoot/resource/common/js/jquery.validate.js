					...
					function handle() {
						if (validator.settings.submitHandler) {
							if (validator.submitButton) {
								// insert a hidden input as a replacement for
	...
	$.extend($.validator, {
		...
		prototype : {
			...
			elements : function() {
				...
				return $(':input', this.currentForm).not(":submit, :reset, :image, [disabled]").not(this.settings.ignore).filter(function() {
				...
				for (method in rules) {
					...
					try {
						if ($.validator.methods[method] != null) {
							...
							// if a method indicates that the field is optional
							// and therefore valid,
							// don't mark it as valid when there are no other
							// rules
