package com.ithxh.xhh.tools.htmlparser;

import org.apache.commons.lang3.StringUtils;
import org.jsoup.helper.StringUtil;

import com.ithxh.xhh.exception.ISBNFormatException;
...
public final class ISBN {
     ...
     * @throws ISBNFormatException 
      */
    public static boolean checkISBN(String isbn) throws ISBNFormatException {

    	if (!StringUtil.isNumeric(isbn)) {
			return false;
		}
		// 判断isbn格式：由978或979开头，不符合
		if (!StringUtils.startsWith(isbn, "97")) {
			return false;
		}
    ...
    public static String to10(String isbn){
        if(isbn.length() == 10){
            return isbn;
        }
        int sum = 0;
        String num=isbn.substring(3,12);
        for(int i = 10, p= 0; i > 1; i--, p++){
            sum += i *Integer.parseInt(num.substring(p, p+1));
        }        
        int m = sum % 11;        
        int check = 11 -  m;        
        if(check == 10){
    ...
    public static String to13(String isbn) {   
    	 if(isbn.length() == 13){
            return isbn;
        }        
        int sum = 0;
        String num = "978"+isbn.substring(0, 9);
        for(int i = 0; i < 12; i++){
